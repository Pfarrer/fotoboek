...
use shared::path_utils::{get_filename, abs_pathbuf_to_rel};
...
pub async fn search_and_update_db(db: FotoboekDatabase, config: &FotoboekConfig) -> SearchAndUpdateResult {
    ...
    let (ok_results, err_results): (Vec<_>, Vec<_>) =
        add_results.iter().partition(|r| r.is_ok());
    err_results.iter().for_each(|r| warn!("Failed to add file: {:?}", r));
...
pub async fn try_add_image(db: &FotoboekDatabase, config: &FotoboekConfig, source_path: &PathBuf) -> Result<(), String> {
    let file = File {
        id: None,
    ...
    if let Some(file) = db.run(move |c| file.insert(c)).await? {
        crate::modules::create_tasks_on_new_image(db, &file)
            .await
